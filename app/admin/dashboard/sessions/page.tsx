...
export default async function SessionsPage({ searchParams }: { searchParams?: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <><PageHeader eyebrow="Real game activity" title="Session history" description="Completed motion-game sessions for users assigned to your Admin account." action={<Link className={styles.secondaryButton} href="/admin/dashboard/reports/sessions.csv">Export CSV</Link>} />
    <Panel title="Completed sessions" subtitle={`${result.total} recorded session${result.total === 1 ? "" : "s"}`}>
      <form className={styles.filterBar} method="get"><input className={styles.filterInput} defaultValue={query} name="q" placeholder="Search session or user" /><select className={styles.filterSelect} defaultValue={activity} name="activity"><option value="all">All activities</option><option value="watering">Watering</option><option value="collect_bugs">Collect Bugs</option><option value="snapshot">Snapshot</option></select><select className={styles.filterSelect} defaultValue={String(days)} name="days"><option value="7">Last 7 days</option><option value="30">Last 30 days</option><option value="42">Last 6 weeks</option><option value="90">Last 90 days</option><option value="365">Last year</option></select><button className={styles.secondaryButton} type="submit">Apply filters</button></form>
...
function validActivity(value?: string): ActivityType | "all" { return value === "watering" || value === "collect_bugs" || value === "snapshot" ? value : "all"; }
