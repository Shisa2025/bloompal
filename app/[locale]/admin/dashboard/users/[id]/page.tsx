...
import { formatDate, formatDateTime, formatDuration } from "@/app/admin/dashboard/_lib/format";
...
export default async function UserDetailPage({
  ...
  const noticeCode = one(query?.notice); const errorCode = one(query?.error);
  ...
  return (
    <>
      <section className={styles.profileHero}>
        <div className={styles.profileIdentity}><Avatar name={user.displayName} size="large" /><div><p className={styles.eyebrow}>{t("managedUser")}</p><h1>{user.displayName}</h1><p>{user.userid} · {user.email}</p><div className={styles.profileBadges}><StatusBadge tone={user.status === "active" ? "success" : "neutral"}>{t(`status.${user.status}`)}</StatusBadge>{user.mustChangePassword ? <StatusBadge tone="warning">{t("passwordChangeDue")}</StatusBadge> : null}</div></div></div>
      ...
      <section className={styles.statsGrid} aria-label={t("userActivityMetrics")}>
        ...
        <article className={styles.motionMetric}><span>{t("lastActivityLabel")}</span><strong className={styles.compactMetric}>{user.lastActivityAt ? formatDate(user.lastActivityAt, locale) : "—"}</strong><small>{user.lastActivityAt ? formatDateTime(user.lastActivityAt, locale) : t("noActivityYet")}</small></article>
        <article className={styles.motionMetric}><span>{t("created")}</span><strong className={styles.compactMetric}>{formatDate(user.createdAt, locale)}</strong><small>{user.lastLoginAt ? t("lastLogin", { date: formatDate(user.lastLoginAt, locale) }) : t("neverSignedIn")}</small></article>
      </section>
      <div className={styles.profileGrid}>
        ...
        <div>
          ...
          <Panel className={styles.sectionGap} title={t("resetPassword")} subtitle={t("resetPasswordDescription")}><form action={resetManagedPasswordAction} className={styles.adminForm}><input name="userid" type="hidden" value={user.userid} /><div className={styles.adminFormField}><label htmlFor="reset-managed-user-password">{t("temporaryPassword")}</label><PasswordInput id="reset-managed-user-password" name="password" minLength={8} required /></div><button className={styles.secondaryButton} type="submit">{t("resetPassword")}</button></form></Panel>
          <Panel className={styles.sectionGap} title={t("accountAccess")} subtitle={t("actionsScoped")}><div className={styles.accountActions}><form action={setManagedUserStatusAction}><input name="userid" type="hidden" value={user.userid} /><input name="status" type="hidden" value={user.status === "active" ? "disabled" : "active"} /><button className={styles.secondaryButton} type="submit">{user.status === "active" ? t("disableSignOut") : t("enableUser")}</button></form><form action={releaseManagedUserAction}><input name="userid" type="hidden" value={user.userid} /><button className={styles.dangerButton} type="submit">{t("releaseFromAdmin")}</button></form></div></Panel>
        </div>
      </div>
    </>
  );
