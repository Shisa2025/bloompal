...
describe("game completion metrics", () => {
  it("accepts the exact repetition targets", () => {
    expect(validWateringMetrics({ sessionId, durationSeconds: 30, leftRepetitions: 5, rightRepetitions: 5 })).toBe(true);
  ...
  it("rejects malformed, incomplete, or implausible payloads", () => {
    expect(validWateringMetrics({ sessionId: "bad", durationSeconds: 30, leftRepetitions: 5, rightRepetitions: 5 })).toBe(false);
